...
**ODM-KV jointly optimizes token eviction and KV-cache precision under an average-bit budget.** Each KV entry receives a bit-width from $\mathcal{B}=\lbrace0,2,3,4,8,16\rbrace$, where $0$ means eviction and $16$ preserves full precision.
